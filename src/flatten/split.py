...
def get_segments(triangles: gpd.GeoSeries):
    ...
    list_of_lists = [[LineString(sorted([ring.coords[i], ring.coords[i+1]])) for i in range(0,len(ring.coords)-1)] for ring in rings]
...
def main(
    ...
    gdf_segments = gpd.GeoDataFrame(pd.DataFrame({'segment_id': list(range(0,len(segments)))}), geometry=segments, crs=gpd1.crs )
    ...
    for feature in gpd2.iterfeatures():
        ...
        line.geom_type
        intersecting_segments = gdf_segments.loc[gdf_segments.intersects(line)]["geometry"]
        intersections = [line.intersection(segment) for segment in intersecting_segments]
        intersection_zipped = zip(intersecting_segments, intersections)
        intersection_zipped = [intersection for intersection in intersection_zipped if intersection[1].geom_type == "Point"]
        # TODO handle MultiPoints?
        if intersection_zipped:
            # print(intersection_zipped)
            distances = [shapely.line_locate_point(line, p[1]) for p in intersection_zipped]
            # unzip
            intersecting_segments, intersections = zip(*intersection_zipped)
            # zip again
            zipped: tuple[list[LineString], list[Point], list[float]] = zip(intersecting_segments, intersections, distances) # type: ignore
            # sort by distance
            sorted_intersections = sorted(zipped, key=lambda intersection: intersection[2], reverse=not direct)
            point: tuple[float, ...] = line.coords[0] if direct else line.coords[-1]
            def update(x: tuple[tuple[float, ...], list[tuple[float, ...]], list[tuple[float, ...]]], y: tuple[LineString, Point, float]):
    ...
    gdf_points = gpd.GeoDataFrame(pd.DataFrame(
        {
            'point_id': list(range(0,len(geoms))),
            'segment_id': segment_id,
            'order': order,
            'position': position
        }), geometry=geoms, crs=gpd1.crs )
