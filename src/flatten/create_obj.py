...
def write_obj(
    ...
    for _, vertex in vertices.items():
        ...
        obj_file.write(
            "v {0} {1} {2}\n".format(
                rescaled_vertex.x, rescaled_vertex.y, rescaled_vertex.z
            )
        )  # type: ignore
    ...
    for triangle in triangles:
        ...
        for coord in coords:
            tol = 1e-9
            mask = (np.abs(vertices.x - coord[0]) < tol) & (
                np.abs(vertices.y - coord[1]) < tol
            )
            matches = vertices[mask]
            if any(matches):
                triangle_indices.append(
                    matches.index[0] + 1
                )  # index start at 1 for obj
        if (
            len(triangle_indices) == 4
        ):  # if one of the points was not found (4 since the first is repeated as last)
            obj_file.write(
                "f {0} {1} {2}\n".format(
...
def export(
    point_geom: gpd.GeoSeries,
    triangles: gpd.GeoSeries,
    output: str,
    output_range_x: float | None = None,
    z_rescale: float = 1.0,
    rescale: bool = True,
):
