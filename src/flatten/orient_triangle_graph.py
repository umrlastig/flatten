...
from flatten.triangle_graph import get_triangle_graph_as_gdf, get_triangle_graph_as_nx
import shapely
...
def split_cycle_on_change(cycle: List[Any], shared_nodes: Set[Any]) -> List[Tuple[List[Any], bool]]:
    ...
    groups = [
        (list(g), flag)
        for flag, g in groupby(shifted, key=is_shared)
    ]
    return groups
...
def split_cycle(cycle: List[Any], split_nodes: Set[Any]) -> List[List[Any]]:
    ...
    return [cycle_sublist(cycle,start,end) for (start, end) in zip(split_indices, split_indices[1:] + split_indices[:1])]
...
def add_path(G: nx.DiGraph, sub_path: List[Any], is_start_outgoing: bool):
    ...
    for i in range(len(sub_path) - 1):
        if not G.has_edge(sub_path[i+1], sub_path[i]):
            G.add_edge(sub_path[i], sub_path[i+1], edge_ids=[], geometry=LineString([G.nodes[sub_path[i]]["geometry"].centroid, G.nodes[sub_path[i+1]]["geometry"].centroid]))
...
def main():
        ...
        for src, dst in zip(seq[:-1], seq[1:]):
            ...
            if G.has_edge(src, dst):
                ...
                if not G.has_edge(dst, src):
                    ...
                    G.add_edge(src, dst, edge_ids=[edge_id], geometry=LineString([src_geometry.centroid, dst_geometry.centroid]))
    ...
    for cycle in triangle_graph_cycles:
        ...
        if len(shared) == len(cycle):
            ...
            if len(shared) == 0:
                ...
                split_nodes = set(filter(lambda n: len(list(triangle_graph.neighbors(n))) > 2, cycle)).intersection(connected_nodes)
    ...
    while len(connected_nodes) < len(G.nodes):
        edges = [(a,b) for (a,b) in triangle_graph.edges if (a in connected_nodes) ^ (b in connected_nodes)]
        for (a, b) in edges:
